...
功能描述：生成一个WAN连接修改工单,对于ADSL上行的CPE,根据PVC进行修改,
    ...
    | ChangeTo1     | 0 | 第一条WAN连接修改后的PVC或VLAN ,默认为ADSL的PVC值:PVC:0/71 |
    | PVC_OR_VLAN_2 | PVC:0/62 | 第二条WAN连接的PVC或VLAN,默认为ADSL的PVC值:PVC:0/62 |
    | ChangeTo2     | 1 | 第二条WAN连接修改后的PVC或VLAN,默认为ADSL的PVC值:PVC:0/72 |
    | PVC_OR_VLAN_3 | null | 第三条WAN连接的PVC或VLAN,默认为null,表示不进行操作 |
    | ChangeTo3     | 1 | 第三条WAN连接启用或禁用标识,默认为ADSL的PVC值:PVC:0/73 |
