...
def _find_wan_server_and_connecttype(sn_obj,
    ...
    if path2_1_list != []:
        ...
        info = u"开始调用GetParameterValues查找WAN连接实例下的X_CT-COM_ServiceList、X_CT-COM_LanInterface和ConnectionType值。\n"
        log.app_info (info)
        ret_data_scr += info
        ...
        if (ret_get_value == ERR_SUCCESS):
            info = u"查找WAN连接实例下的X_CT-COM_ServiceList、X_CT-COM_LanInterface和ConnectionType值成功,返回:%s 。\n" % ret_data_getvalue
            log.app_info (info)
            ret_data_scr += info
                        
        else:
                        
            info = u"获取WAN连接实例下的X_CT-COM_ServiceList、X_CT-COM_LanInterface和ConnectionType值失败，错误信息：%s 。\n" % ret_data_getvalue
            log.app_err (info)
            ret_data_scr += info
            return ret_res, serverlist_items,connect_type_items,ret_data_scr            
