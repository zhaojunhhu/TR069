...
def IPTVDisable(obj, sn, DeviceType, AccessMode,
                PVC_OR_VLAN, dict_root,
    ...
    for nwf in [1]:
        try:
            ...
            # 第一步：调用SetParameterValues设置X_CU_IPTV参数：
            info = u"开始调用SetParameterValues设置X_CU_IPTV参数\n"
            log.app_info(info)
            ret_data_scr += info
            ...
            for i in dict_root:
                if dict_root[i][0] == 1:
                    tmp_path = path + i
                    para_list.append(dict(Name=tmp_path, Value=dict_root[i][1]))
            if para_list == []:
