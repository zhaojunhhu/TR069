        ...
        try:
            ...
            if WAN_Flag == 0:
                ...
                if serverlist_items != []:
                    ...
                    for i_serverlist in serverlist_items:
                        ...
                        if tmp_link_path in i_serverlist['Name']:
                            ...
                            if i_serverlist['Value'] == tmp_X_CU_ServiceList:
                    ...
                    for i_laninterface in laninterface_items:
                        ...
                        if tmp_link_path in i_laninterface['Name']:
                            ...
                            if i_laninterface['Value'] == tmp_X_CU_LanInterface:
                ...
                if WAN_Flag_connect != 3:
                    ...
                    if WAN_Flag_interface == 0 or WAN_Flag_server == 0:
                        ...
                        # IGMP参数
                        path = 'InternetGatewayDevice.Services.X_CU_IPTV.'
                        for i in dict_root:
                            if dict_root[i][0] == 1:
                                tmp_path = path + i
            ...
            if WAN_Flag == 1:
                ...
                if DeviceType == 'ADSL':
                    ...
                    path5 = tmp_path3
                    info = u"开始调用SetParameterValues设置X_CU_VLAN，WANPPPConnection（或WANIPConnection）及IGMP参数。\n"
...
def _find_wan_server_and_connecttype(sn_obj, DeviceType,
    ...
    if DeviceType == 'ADSL':
        ...
        reg_VLAN = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANEthernetLinkConfig.X_CU_VLANIDMark$"
    elif DeviceType == 'EPON':

        reg_VLAN = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.X_CU_VLAN$"
    elif DeviceType == 'VDSL':

        reg_VLAN = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANDSLLinkConfig.X_CU_VLAN$"
    elif DeviceType == 'GPON':
    ...
    if len(ret_connectiontype) != len(ret_serverlist) and len(ret_connectiontype) != len(ret_laninterface):
        info = u"GetParameterNames查找到WAN连接的X_CU_ServiceList,ConnectionType和X_CU_LanInterface节点个数不等，工单异常退出。\n"
        log.app_info(info)
        ret_data_scr += info
    ...
    if path2_1_list:
        ...
        info = u"开始调用GetParameterValues查找WAN连接实例下的VLANID、X_CU_ServiceList,ConnectionType和X_CU_LanInterface值。\n"
        log.app_info(info)
        ret_data_scr += info
        ...
        if ret_get_value == ERR_SUCCESS:
            info = u"查找WAN连接实例下的VLANID、X_CU_ServiceList,ConnectionType和X_CU_LanInterface值成功。\n"
            log.app_info(info)
            ret_data_scr += info

        else:

            info = u"获取WAN连接实例下的VLANID、X_CU_ServiceList，ConnectionType和X_CU_LanInterface值失败，错误信息：%s 。\n" % ret_data_getvalue
            log.app_err(info)
            ret_data_scr += info
