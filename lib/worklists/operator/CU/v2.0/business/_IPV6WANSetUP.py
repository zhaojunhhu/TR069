...
def V6WANSetUP(obj, sn, WANEnable_Switch, DeviceType,
    ...
    for nwf in [1]:
        try:
            ...
            log.app_info(info)
            ret_data_scr += info
            ret_device_info, ret_data_device_info = u1.get_parameter_names(ParameterPath=DeviceInfo_Root_Path,
                                                                           NextLevel=0)
            ...
            #     if tmp_path == "InternetGatewayDevice.DeviceInfo.X_CU_InterfaceVersion":
            ...
            #     ret_data_scr += info
            #
            #     ret_interface_version, ret_data_interface_version = u1.get_parameter_values(
            #         ParameterNames=InterfaceVersion_Path)
            #
            ...
            IPProtocolVersion_Path = "InternetGatewayDevice.DeviceInfo.X_CU_IPProtocolVersion"
            ...
            PrefixInformation_Path = "InternetGatewayDevice.LANDevice.1.X_CU_IPv6Config.PrefixInformation."
            info = u"开始调用GetParameterNames方法，查询PrefixInformation信息。\n"
            log.app_info(info)
            ret_data_scr += info
            ret_prefix_info, ret_data_prefix_info = u1.get_parameter_names(ParameterPath=PrefixInformation_Path,
                                                                           NextLevel=0)
            ...
            # 保存WANPPPConnection或WANIPConnection节点路径保存,后面修改参数时有用
            # 是否有查到已存在的类似WAN连接的状态位,符合以下标识,桥只关心PVC,其他的只关心X_CU_ServiceList

            X_CU_ServiceList1 = None
            X_CU_ServiceList2 = None
            ...
            if AccessMode1 == 'PPPoE' or AccessMode1 == 'DHCP' or AccessMode1 == 'Static' or AccessMode1 == 'PPPoE_Bridged':
                ...
                if AccessMode1 == 'PPPoE' or AccessMode1 == 'PPPoE_Bridged':
                    ...
                    reg_X_CU_ServiceList1 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANPPPConnection.\d+\.X_CU_ServiceList"
                    reg_ConnectionType1 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANPPPConnection.\d+\.ConnectionType"

                    X_CU_ServiceList1 = dict_wanpppconnection1['X_CU_ServiceList'][1]
                    ...
                    reg_X_CU_ServiceList1 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANIPConnection.\d+\.X_CU_ServiceList"
                    reg_ConnectionType1 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANIPConnection.\d+\.ConnectionType"

                    X_CU_ServiceList1 = dict_wanipconnection1['X_CU_ServiceList'][1]
            ...
            if AccessMode2 != '':
                if AccessMode2 == 'PPPoE' or AccessMode2 == 'DHCP' or AccessMode2 == 'Static' or AccessMode2 == 'PPPoE_Bridged':
                    ...
                    if AccessMode2 == 'PPPoE' or AccessMode2 == 'PPPoE_Bridged':
                        ...
                        reg_X_CU_ServiceList2 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANPPPConnection.\d+\.X_CU_ServiceList"
                        reg_ConnectionType2 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANPPPConnection.\d+\.ConnectionType"

                        X_CU_ServiceList2 = dict_wanpppconnection2['X_CU_ServiceList'][1]
                        ...
                        reg_X_CU_ServiceList2 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANIPConnection.\d+\.X_CU_ServiceList"
                        reg_ConnectionType1 = "InternetGatewayDevice\.WANDevice\.1\.WANConnectionDevice.\d+\.WANIPConnection.\d+\.ConnectionType"

                        X_CU_ServiceList2 = dict_wanipconnection2['X_CU_ServiceList'][1]

                    ret_find, serverlist_items2, connect_type_items2, ret_data_find = _find_wan_server_and_connecttype(
                        u1,
                        AccessMode2,
                        dict_wanpppconnection2,
                        dict_wanipconnection2,
                ...
                for j_serverlist in serverlist_items1:
                    ...
                    if tmp_ppp_path in tmp_serverlist:
                        ...
                        if ConnectionType1 in i_connect['Value']:
                            ...
                            if X_CU_ServiceList1 in j_serverlist['Value'] or j_serverlist[
                                'Value'] in X_CU_ServiceList1:

                                modify_path_1 = j_serverlist['Name']
                                if j_serverlist['Value'] in X_CU_ServiceList1:
                    ...
                    for j_serverlist in serverlist_items2:
                        ...
                        if tmp_ppp_path in tmp_serverlist:
                            ...
                            if ConnectionType2 in i_connect['Value']:
                                ...
                                if X_CU_ServiceList2 in j_serverlist['Value'] or j_serverlist[
                                    'Value'] in X_CU_ServiceList2:

                                    modify_path_2 = j_serverlist['Name']
                                    if j_serverlist['Value'] in X_CU_ServiceList2:
            ...
            if not X_CU_ServiceList2:
                ...
                if WAN1_Flag == None:
                    ...
                    if (("tr069" in X_CU_ServiceList1) or
                            ("TR069" in X_CU_ServiceList1)):
                    ...
                    ret6_1, ret_data6_1, reboot_Yes, WAN_Enable = _set_ppporip_para(u1, tmp_values1, path6,
                                                                                    ret_data_scr, WAN1_Flag,
                                                                                    WANEnable_Switch, WAN_Enable)
                ...
                # 一条新建，一条修改
                    ...
                    if WAN1_Flag != None and WAN2_Flag == None:
                        ...
                        match_server = X_CU_ServiceList1
                        ...
                        match_server = X_CU_ServiceList2
...
def _set_link_parameter_values(sn_obj, DeviceType, path2, dict_wanlinkconfig, ret_data_scr, add=False):
    ...
    if DeviceType == 'ADSL':
        ...
        if add == False:
            info = u"走X_CU_ServiceList相同的流程(修改WAN连接),开始调用SetParameterValues修改X_CU_VLAN参数。\n"
        else:
            info = u"开始调用SetParameterValues设置X_CU_VLAN参数。\n"
    ...
    for i in dict_wanlinkconfig:
        if dict_wanlinkconfig[i][0] == 1:
            ...
            if i == 'DestinationAddress' or i == 'VLANIDMark' or \
               i == 'X_CU_VLANIDMark' or i == "X_CU_Mode":
                continue
            """
            tmp_path = path5 + i
...
def _set_ppporip_para(sn_obj, tmp_values, path6_x, ret_data_scr, WAN_Flag, WANEnable_Switch, WAN_Enable):
    ...
    for i in tmp_values:
        if tmp_values[i][0] == 1:
            # 判断X_CU_ServiceList包含与被包含的关系做区分处理.
            # 标志位1表示X_CU_ServiceList值需以工单中的为准,重新下发,2表示不下发
            if WAN_Flag == None:
                pass
            elif WAN_Flag == 1:
                pass
...
def _ipv6_config(sn_obj, ret_data_prefix_info,
    ...
    LAN_IPv6Config_Path = "InternetGatewayDevice.LANDevice.1.X_CU_IPv6Config."
    LAN_DHCPv6_Path = "InternetGatewayDevice.LANDevice.1.X_CU_DHCPv6Server."
    LAN_RouterAdv_Path = "InternetGatewayDevice.LANDevice.1.X_CU_RouterAdvertisement."

    para_list_v6 = []  # 保存LAN侧 v6相关设置
    # PrefixInformation相关
    tmp_prefix_info = []
    re_prefix = "InternetGatewayDevice\.LANDevice\.1\.X_CU_IPv6Config\.PrefixInformation\.\d\.$"
    ...
    # X_CU_IPv6Config相关不包括PrefixInformation
...
def _find_wan_server_and_connecttype(sn_obj,
    ...
    if path2_1_list != []:
        ...
        info = u"开始调用GetParameterValues查找WAN连接实例下的Username、X_CU_ServiceList和ConnectionType值。\n"
        log.app_info(info)
        ret_data_scr += info
        ...
        if ret_get_value == ERR_SUCCESS:
            info = u"查找WAN连接实例下的Username、X_CU_ServiceList和ConnectionType值成功,返回:%s 。\n" % ret_data_getvalue
            log.app_info(info)
            ret_data_scr += info

        else:

            info = u"获取WAN连接实例下的Username、X_CU_ServiceList和ConnectionType值失败，错误信息：%s 。\n" % ret_data_getvalue
            log.app_err(info)
            ret_data_scr += info
            return ret_res, serverlist_items, connect_type_items, ret_data_scr
