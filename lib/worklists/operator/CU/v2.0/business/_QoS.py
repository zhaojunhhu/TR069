...
def QoS(obj, sn, DeviceType, dict_root,
    ...
    # QoS的根节点
    QoS_ROOT_PATH = "InternetGatewayDevice.X_CU_Function.UplinkQoS."
    for nwf in [1]:
        try:
            ...
            for i in xrange(len(list_value_type)):
                ...
                # InternetGatewayDevice.X_CU_Function.UplinkQoS.Classification.
                info = u"开始调用AddObject新建Classification实例\n"
                log.app_info(info)
                ret_data_scr += info
                Classification_path = "InternetGatewayDevice.X_CU_Function.UplinkQoS.Classification."
                # sleep(3)  # must be ;otherwise exception
