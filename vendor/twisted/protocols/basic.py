...
class LineReceiver(protocol.Protocol, _PauseableMixin):
    ...
    def dataReceived(self, data):
        ...
        """     
        # nwf 2013-10-30; save head
        self.save_in_head(data)
    ...
    def save_in_head(self, data):
        """
        """
        pass
        tag = self.delimiter * 2
        index1 = data.find(tag)
        # is head?
        if (index1 == -1):
            return
        # auto add line=\r\n
        self.content_head_in = data[:index1+len(tag)]
